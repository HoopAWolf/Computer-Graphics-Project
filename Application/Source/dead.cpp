...
void dead::Init()
{
	...
	meshList[GEO_FRONT] = MeshBuilder::GenerateOBJ("", "OBJ//deathscreen.obj");
	meshList[GEO_FRONT]->textureID = LoadTGA("Image//deathscreen.tga");
...
void dead::Render()
{
	...
	RenderMeshOnScreen(meshList[GEO_FRONT], 40, 30, 30, 25, 90);
...
void dead::RenderMesh(Mesh *mesh, bool enableLight)
{
...
void dead::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void dead::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
...
//============================================TESTING===============================================
void dead::RenderUI(Mesh* mesh, Color color, float size, float x, float y, bool enableLight)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
...
void dead::RenderMeshOnScreen(Mesh* mesh, float x, float y, int sizex, int sizey, int rotate)
{
	glDisable(GL_DEPTH_TEST);
	Mtx44 ortho;
	ortho.SetToOrtho(0, 80, 0, 60, -10, 10); //size of screen UI
